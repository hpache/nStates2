    ...
    def _createOmegaMatrix(self) -> list:
        ...
        for i in range(numberGround):
            for j in range(numberInter):
                ...
                omega[i][j + numberGround] = 1/2*ground_inter
                omega[j + numberInter][i] = 1/2*ground_inter
        ...
        for i in range(numberInter):
            for j in range(numberExcited):
                ...
                omega[i + numberGround][j + numberGround + numberInter] = 1/2*inter_exc
                omega[j + numberGround + numberInter][i + numberGround] = 1/2*inter_exc
